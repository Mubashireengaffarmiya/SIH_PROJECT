...
def _normalize(value: Any) -> str:
    ...
    text = re.sub(r"\b(?:rs\.?|inr|rupees?)\b|₹", "", text)
...
def build_hybrid_extraction(ocr_extraction: Dict[str, Any], vlm_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for ocr_key, vlm_key in FIELD_MAP.items():
        ...
        if ocr_value and vlm_value and _normalize(ocr_value) == _normalize(vlm_value):
            status = "AGREED"
        ...
        result[ocr_key] = {
            ...
            "status": status,
            "needs_review": status in {"CONFLICT", "NOT_DETECTED"},
        }
    return result